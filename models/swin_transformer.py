...
class PatchUpsample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = self.upsample(x) # B L 2C

        x0, x1, x2, x3 = x.view(B, L, C//2, 4).split(1, dim=-1)
        x0 = x0[:, :, :, 0].view(B, H, W, C//2).permute(0,3,1,2)
        x1 = x1[:, :, :, 0].view(B, H, W, C//2).permute(0,3,1,2)
        x2 = x2[:, :, :, 0].view(B, H, W, C//2).permute(0,3,1,2)
        x3 = x3[:, :, :, 0].view(B, H, W, C//2).permute(0,3,1,2)
        x01 = torch.stack([x0, x1], dim=4).reshape(B, C//2, H, 2*W)
        x23 = torch.stack([x2, x3], dim=4).reshape(B, C//2, H, 2*W)
        x0123 = torch.stack([x01, x23], dim=3).reshape(B, C//2, H, 4*W)
        x = x0123.reshape(B, C//2, 4*L).permute(0,2,1)
...
class PatchEmbed_Bolin(nn.Module):
    r""" Image to Patch Embedding
    Args:
        img_size (int): Image size.  Default: 224.
        patch_size (int): Patch token size. Default: 4.
        ...
        norm_layer (nn.Module, optional): Normalization layer. Default: None
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, first_bias=False):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]
        ...
        self.out_ch = out_ch
        self.conv1 = nn.Conv2d(in_ch, hidden_ch, kernel_size=3, stride=2, padding=1, bias=first_bias)
        self.norm1 = nn.BatchNorm2d(hidden_ch)
        #self.norm1 = nn.SyncBatchNorm(hidden_ch)
        self.conv2 = nn.Conv2d(hidden_ch, hidden_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm2 = nn.BatchNorm2d(hidden_ch)
        #self.norm2 = nn.SyncBatchNorm(hidden_ch)
    ...
    def flops(self):
        ...
        Ho = H//2
        Wo = W//2
        flops += Ho * Wo * self.hidden_ch * self.in_chans * (3**2)
        ## norm1
        flops += Ho * Wo * self.hidden_ch
        ## conv2
        flops += Ho * Wo * self.hidden_ch * self.hidden_ch * (3**2)
        ## norm2
        flops += Ho * Wo * self.hidden_ch
        ## conv3
        Ho = Ho // 2
        Wo = Wo // 2
        flops += Ho * Wo * self.hidden_ch * self.out_ch * (3**2)
        if self.norm is not None:
            flops += Ho * Wo * self.embed_dim
        return flops
...
class PatchRecover(nn.Module):
    ...
    def __init__(self, input_resolution, out_ch=3, embed_dim=96):
        ...
        self.conv1 = nn.Conv2d(in_chans, hidden_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm1 = nn.BatchNorm2d(hidden_ch)
        #self.norm1 = nn.SyncBatchNorm(hidden_ch)
        self.conv2 = nn.Conv2d(hidden_ch, hidden_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm2 = nn.BatchNorm2d(hidden_ch)
        ...
        self.conv3 = nn.Conv2d(hidden_ch, hidden_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm3 = nn.BatchNorm2d(hidden_ch)
        self.conv4 = nn.Conv2d(hidden_ch, out_ch, kernel_size=1, stride=1, bias=True)
        self.act = nn.ReLU(inplace=True)
...
class SwinEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
        ...
        self.patches_resolution = patches_resolution
        self.output_patches_resolution = [patches_resolution[0] // (2**(self.num_layers-1)),
...
class SwinDecoder(nn.Module):
    ...
    def __init__(self, input_resolution, patch_size=4, out_ch=3,
        ...
        ## Reverse the list for decoder part
        depths = depths[::-1]
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = BasicLayer_Decoder(dim=int(embed_dim / 2 ** i_layer),
